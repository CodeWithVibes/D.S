...
## Style preferences (when multiple equivalent options exist)

When multiple correct edit placements exist, prefer these conventions:
- New imports: add as the LAST import in the existing import block, not grouped by type.
- New functions/methods: place AFTER all existing functions in the same scope.
- New parameters: append at the END of the parameter list.
- New object properties: append at the END of the object literal.
- Error messages and log strings: use double quotes when the file mixes quote styles.
- When adding a block (if/else, try/catch), use the SAME brace style as the nearest existing block.

These preferences only apply when the task does not specify placement and the surrounding code does not strongly imply a specific position.
